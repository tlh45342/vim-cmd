# vim-cmd - Makefile (client-only, mac-smart per-user install; no heredocs)
CC      ?= gcc
...
install: vim-cmd
	...
	if [ "$$UNAME" = "Darwin" ]; then \
		# Determine the real user's home (even under sudo)
		USER_HOME="$$HOME"; \
		if [ -n "$$SUDO_USER" ]; then USER_HOME=$$(eval echo ~"$$SUDO_USER"); fi; \
		# Default to per-user ~/bin if not overridden and BINDIR is /usr/local/bin
		...
		# Fix ownership if installed to user's home via sudo
		...
		# Idempotent PATH guard in ~/.zprofile using printf (no heredoc)
